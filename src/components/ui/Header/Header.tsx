...
export const Header = () => {
  ...
  return (
    <>
          ...
          <div className="flex items-center min-w-fit">{/* Right section */}
            {
              isLoggedIn ? (
                ...
                <div className="flex gap-2 w-40 items-center">
                  ...
                  <button className="flex-1 bg-blue-600 text-white px-3 py-1 rounded text-sm text-nowrap font-bold" onClick={() => setOpenLogin(true)}>
                    Login
                  </button>
                </div>
            }
      ...
      { openLogin ? (
        <Modal title={"Login"} onClose={() => setOpenLogin(false)} isOpen={openLogin} content={loginContent} />
      ) : null
      }
    </>
  );
